...
async function loadResources(assignmentId) {
    ...
    try {
        ...
        container.innerHTML = data.map(res => {
            ...
            const finalURL = filePath.startsWith('http') ? filePath : `https://sigra-backend.onrender.com/${filePath.replace(/\\/g, '/')}`;
...
async function loadActivities(assignmentId) {
    ...
    try {
        ...
        const resSubmissions = await fetch(`https://sigra-backend.onrender.com/api/submissions/students/${STUDENT_ID}/submissions`);
